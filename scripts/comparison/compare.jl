...
pplt.close(); f,axs = pplt.subplots(
    ...
    wspace=[0,nothing,0,nothing,0,nothing,0]
);
...
axs[2].plot(DTP_2DH.-ocn_DTP,elvl)
axs[2].plot(DTP_LSV.-ocn_DTP,elvl)
axs[2].plot(DTP_SHR.-ocn_DTP,elvl)
...
_,IPW_WTG = preinterp(svar,"WTG","IPW1M",elvl)
axs[3].plot(ocn_IPW,elvl,c="k"); axs[3].format(title="IPW_OCN")
axs[4].plot(IPW_2DH.-ocn_IPW,elvl)
axs[4].plot(IPW_LSV.-ocn_IPW,elvl)
axs[4].plot(IPW_SHR.-ocn_IPW,elvl)
axs[4].plot(IPW_WTG.-ocn_IPW,elvl)
...
_,WPW_WTG = preinterp(svar,"WTG","WPW1M",elvl)
axs[5].plot(ocn_WPW,elvl,c="k"); axs[5].format(title="WPW_OCN")
axs[6].plot(WPW_2DH.-ocn_WPW,elvl)
axs[6].plot(WPW_LSV.-ocn_WPW,elvl)
axs[6].plot(WPW_SHR.-ocn_WPW,elvl)
axs[6].plot(WPW_WTG.-ocn_WPW,elvl)
...
_,DRY_WTG = preinterp(svar,"WTG","DRY1M",elvl)
axs[7].plot(ocn_DRY,elvl,c="k"); axs[7].format(title="DRY_OCN")
axs[8].plot(DRY_2DH.-ocn_DRY,elvl)
axs[8].plot(DRY_LSV.-ocn_DRY,elvl)
axs[8].plot(DRY_SHR.-ocn_DRY,elvl)
axs[8].plot(DRY_WTG.-ocn_DRY,elvl)
...
for ii = 1 : 2 : 8
    ...
    axs[ii+1].format(
        abc=true,grid="on",ylim=(1000,70),
        xlim=(-20,20),xlabel="Temperature / K",
        ylabel="Pressure / hPa"
    )
end
