...
p,mocca_RCE,vocca_RCE = sam3D("Control2DHR","DTP1M")
p,mocca_LSV,vocca_LSV = sam3D("LSVert","DTP1M")
p,mocca_SHR,vocca_SHR = sam3D("Shear","DTP1M")
# p,mocca_WTG,vocca_WTG = sam3D("WTG","DTP1M")
...
axs[3].plot(mocca_RCE[:],lvl)
axs[4].contourf(0:24,lvl,vocca_RCE',cmap="drywet",levels=clvl,extend="both")
axs[3].format(urtitle="RCE")

axs[5].plot(mocca_LSV[:],lvl)
axs[6].contourf(0:24,lvl,vocca_LSV',cmap="drywet",levels=clvl,extend="both")
axs[5].format(urtitle="LSVert")

axs[7].plot(mocca_SHR[:],lvl)
axs[8].contourf(0:24,lvl,vocca_SHR',cmap="drywet",levels=clvl,extend="both")
axs[7].format(urtitle="Shear")

# axs[9].plot(mocca_WTG[:],lvl)
# axs[10].contourf(0:24,lvl,vocca_WTG',cmap="drywet",levels=clvl,extend="both")
